...
| FR-SIS-09 | Every student shall carry a **year level**, derived rather than typed: a new enrollee enters at year 1, a transferee at the level their credited units earn, and a returning student moves up when the school year turns over — overridable by staff, and audited. | ✅ |
| FR-SIS-10 | Term activation shall identify a returning student by their **official student number and last name** — the number on their ID, not SEN-GEN's internal registration number. | ✅ |
...
*FR-SIS-09/10 built (migration `AddTransfereeEvaluationAndYearLevel`). **Year level** is `StudentRegistration.YearLevel` and the rules live in one place, `Features/Registration/YearLevelPolicy`: `ForNewRegistration` → 1 (a transferee starts there too, so an unevaluated one is never silently placed higher); `FromCreditedUnits` measures credited units against the curriculum's own per-year unit totals, so a 60-unit-a-year program and a 42-unit-a-year one are each judged on their own terms; `OnTermActivation` advances on the **school year** changing, not the semester — the 2nd semester of year 2 is still year 2. Every path is overridable by staff and every change is audited (`YearLevelAssigned`). **Term activation** now looks the student up by `OfficialStudentNumber` first, falling back to the registration number so anyone never issued a student number is not locked out; the response echoes back whichever number identified them, and the staff queue shows the official number, the last name, and the year level the approval will settle. Validation is where a returning student's year is assigned — derived by default, `yearLevel` on the request overriding it for a repeating student or one back from a leave.*
...
| FR-ENL-11 | The Registrar shall be able to **decide many slot requests in one action** — a selection, a student's whole outstanding load, or the entire pending queue — without weakening the per-request checks. | ✅ |
| FR-ENL-12 | A **transferee** shall not enlist until the Registrar has completed their credit evaluation (FR-TRAN). | ✅ |
...
### FR-TRAN — Transferee Credit Evaluation

| ID | Requirement | Status |
|----|-------------|:------:|
| FR-TRAN-01 | The Registrar shall evaluate a transferee **subject by subject** against the curriculum they are entering, marking each Credited (recording the previous school's subject and grade) or To take. | ✅ |
| FR-TRAN-02 | A completed evaluation shall set the transferee's **year level** from their credited units and open their enlistment gate; until it is completed they cannot enlist. | ✅ |
| FR-TRAN-03 | The evaluation shall be printable as a **signable record**, and correctable after sign-off without losing the decisions already made. | ✅ |

*Built: `Features/Registration/TransfereeEvaluation` (`/api/transferee-evaluations`, Registrar + Admission Officer + School Admin) and `Domain/TransfereeEvaluation` + `TransfereeEvaluationItem` (migration `AddTransfereeEvaluationAndYearLevel`). A transferee arrives carrying subjects passed somewhere else, and someone has to rule which of them count here — a ruling that settles two things at once: **which subjects are still theirs to take**, and **what year they enter at**. That is why the evaluation is a hard gate in `EnlistmentEligibility` rather than a report: before it exists there is no honest answer to what a transferee would even be enlisting in.*

*The sheet is the student's whole curriculum in curriculum order, one verdict per subject; a credited row records what it was credited **from** (subject + grade), because a credit nobody can trace back is a credit nobody can defend. Saving is incremental (`Pending → InProgress`) since a long curriculum is worked through over more than one sitting, and completing requires **every** subject to have a verdict — a sheet with gaps cannot answer the question it exists to answer. On completion `YearLevelPolicy.FromCreditedUnits` derives the year level, the Registrar may override it, and both the derivation and the assignment are kept (`RecommendedYearLevel` / `AssignedYearLevel`) so a later override never erases what the curriculum actually implied. Reopening keeps the decisions — a correction is a revision, not a fresh start — and closes the enlistment gate again until it is signed off. The curriculum is **pinned** on the evaluation, so a signed sheet is never silently re-read against a newer catalog. Audited as `TransfereeEvaluationSaved` / `Completed` / `Reopened` plus `YearLevelAssigned`; the student gets a bell notice on completion.*

*Client `/evaluate-transferee` (sidebar, badge-counted): a queue of transferees with how far each has got, and a sheet modal that rules term block by term block ("Credit all" / "All to take" for the common wholesale case), shows credited units and the derived year level moving **as the decisions are made**, and blocks Complete while anything is undecided. The student sees the evaluation as its own step on their dashboard journey, placed before enlistment exactly as document clearance is for a new enrollee, and never sees it at all if they are not a transferee.*
...
| FR-PUB-04 | A class changed **after** it was published shall be recorded as an **amendment** — flagged on the row, and notified to the faculty member and every student holding a seat in it. | ✅ |
...
*FR-PUB-04 built (migration `AddTransfereeEvaluationAndYearLevel`). Publishing is not a status flag — it is a promise already emailed to a faculty member and to every student holding a seat, and until now the board could move or delete a published class with **nobody being told**. Rooms flood and staff change, so the edit stays allowed; what changes is that it is no longer silent. `Features/Scheduling/Board/ScheduleAmendments` flags the row (`ScheduleAssignment.IsAmended` / `AmendedAtUtc` / `AmendedByUserId`), audits `ScheduleAmended`, and notifies the assigned faculty member plus every student with an **approved** slot request in that section — a bell notice committed in the same transaction as the edit, and a best-effort email sent after it, so a mail outage can never roll back a change the board already accepted. The notice states the move in plain terms ("moved from Monday 08:00–11:00 in Room 201 to …"). The board asks for confirmation before an edit that will reach people, marks amended classes with a dashed outline, and the certificate of registration prints the same flag. Draft rows are untouched by all of this — they were never promised to anyone.*
...
| FR-RPT-06 | The system shall produce **printable subject listings** as PDFs: the curriculum prospectus for a year level, a transferee's credit-evaluation sheet, and a student's certificate of registration. | ✅ |
...
*FR-RPT-06 built: `Features/Reports/Prospectus` (`/api/prospectus/*`, QuestPDF) — three documents answering "what subjects does this person take?" at three scopes. The **curriculum prospectus** (`curriculum.pdf?curriculumId=&yearLevel=`) prints a year level's subjects term by term with units, the lecture/laboratory split, prerequisites, and running totals; omit `yearLevel` for the whole ladder. The **transferee evaluation sheet** (`students/{id}/evaluation.pdf`) prints the credit ruling as a signable record — credited subjects with what they were credited *from*, the remaining load, the assigned year level, and Evaluated-by/Conforme blocks. The **certificate of registration** (`students/{id}/registration-form.pdf`) prints the subjects a student holds an approved seat in, with day/time/room/faculty and total units; a class amended after publication is flagged on it, so a printed copy carries the same warning the screen does. Students reach their own two at `/api/prospectus/me/*` — resolved from the signed-in account rather than an id, so there is no id to tamper with — and an evaluated transferee's curriculum copy comes back with their credited subjects shaded and marked. Client: `/prospectus` (staff) and `/my-subjects` (student); downloads go through a blob fetch because every route is bearer-authenticated.*
